...
Deploys the hotline-listing FastAPI service as a Docker container on the target host and wires it into the nginx reverse proxy at its own subdomain, `{{ hotline_listing_domain }}`.
...
8. Purges the Cloudflare cache (when `cf_purge_cache: true`)
...
| `hotline_listing_root_path` | `''` | URL prefix passed to the app as `ROOT_PATH` — empty since the app now owns its subdomain root |
| `hotline_listing_domain` | `''` | Subdomain the service is served from, e.g. `hotline-listing.zelgray.work` — must also be added to infra's `host_domains` and to `meow-elite-club-portal`'s `meow_elite_club_portal_gated_domains` allowlist |
